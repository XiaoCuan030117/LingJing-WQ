# 最小 Coding Agent（第一阶段）

使用 Python 普通函数和直接模型 SDK，实现自然语言任务 → 模型工具调用 → 实际执行 → 结果回传 → 最终回答。提供单轮 CLI，支持读取 UTF-8 文件、写入文件和运行 Shell。写入和 Shell 均需逐次确认。WebUI、`ask_user` 属于后续阶段，本阶段尚未实现。
...
$env:OPENAI_API_KEY = '<公司提供的密钥>'
$env:OPENAI_BASE_URL = 'https://<公司提供的地址>/v1'
$env:OPENAI_MODEL = '<支持工具调用的模型名称>'
```

配置密钥时暂停录屏，完成后恢复。`.env.example` 只说明变量名称，不会自动加载；不要把真实密钥写入该文件。`OPENAI_BASE_URL` 可省略以使用 SDK 的默认服务地址，模型名称必须明确设置。公司接口须支持 Chat Completions 的 `tools`、`tool_calls` 和 `tool_call_id`，以实际连通验证为准。协议参考：[OpenAI 官方工具调用文档](https://developers.openai.com/api/docs/guides/function-calling)。
...
状态为 `idle → running → waiting_approval → running → completed/failed`。`advance()` 在授权等待期间不会请求模型或重放调用；`approve(token, allowed)` 只消费当前令牌。一个实例运行一个任务，失败后创建新实例；不持久化会话。
